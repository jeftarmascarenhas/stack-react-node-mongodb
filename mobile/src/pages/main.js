...
function Main({ navigation }) {
  ...
  const _keyboardDidShow = event => {
    ...
    setHeightKeyBoard(endCoordinates.height + 20);
  };
  ...
  useEffect(() => {
    const keyoardShowListener = Keyboard.addListener(
      "keyboardDidShow",
      _keyboardDidShow
    );

    const keyoardHiddenListener = Keyboard.addListener(
      "keyboardDidHide",
      _keyboardDidHide
    );
    return () => {
      keyoardShowListener.remove();
      keyoardHiddenListener.remove();
    };
  ...
  return (
    <>
      ...
      <View style={styles.searchForm(heightKeyBoard)}>
        <TextInput
